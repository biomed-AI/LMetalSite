...
def predict(run_id, ID_list, seq_list, protein_features, config, outpath, pred_bs, device):
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_dataloader):
            ...
            outputs = [model(protein_feats, protein_masks).sigmoid() for model in models]
            outputs = torch.stack(outputs).mean(0)   # average the predictions from 5 models
            outputs = outputs.detach().cpu().numpy() # shape = (pred_bs, len(metal_list) * maxlen)
...
def main(run_id, seq_info, outpath, feat_bs, pred_bs, save_feat, gpu):
    ...
    protein_features = feature_extraction(ID_list, seq_list, outpath, feat_bs, save_feat, device)
    ...
    predict(run_id, ID_list, seq_list, protein_features, NN_config, outpath, pred_bs, device)
...
if __name__ == '__main__':
    ...
    if seq_info == -1:
        ...
        main(run_id, seq_info, outpath, args.feat_bs, args.pred_bs, args.save_feat, args.gpu)
